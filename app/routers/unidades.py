...
@router.get("/{id}")
def get_unidade(id: str, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    result = unidade_service.get_by_id(db, id)
    return {"success": True, "message": "Unidade encontrada", "data": result}

@router.post("/")
def create_unidade(data: UnidadeCreate, db: Session = Depends(get_db), current_user = Depends(require_admin)):
    result = unidade_service.create(db, data)
    return {"success": True, "message": "Unidade criada", "data": result}

@router.put("/{id}")
def update_unidade(id: str, data: UnidadeUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
def deactivate_unidade(id: str, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
    result = unidade_service.deactivate(db, id)
    return {"success": True, "message": "Unidade desativada", "data": result}

@router.post("/{id}/grupos")
def create_grupo_in_unidade(id: str, data: GrupoCreate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
...
def update_grupo_in_unidade(uid: str, gid: str, data: GrupoUpdate, db: Session = Depends(get_db), current_user = Depends(get_current_user)):
